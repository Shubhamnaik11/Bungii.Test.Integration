...
namespace Bungii.Test.Integration.Framework.Core
{
    class AssertionManager
    {
        ...
        public static void NotDisplayed(IWebElement locator)
        {
            Assert.IsFalse(element.Displayed, "Element was Displayed");
        }


        public static void CompareMaxLength(IWebElement locator, String value)
        {
            Assert.AreEqual(value, locator.GetAttribute("maxlength"), "Values do not match");
        }

        public static void Selected(IWebElement locator)
        {
            Assert.IsTrue(locator.Selected, "Element was not selected");
        }
